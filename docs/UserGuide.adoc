...
• User inputs the warning amount for the ingredient `w/INGREDIENT_WARNING_AMOUNT` , and when the quantity in inventory falls below this stated amount, there is a need for restock of this ingredient.
Calling `listIngredients w/TRUE` will display all ingredients that fall below their corresponding warning values.
****
...
Format: `depleteIngredient i/INGREDIENT_INDEX q/INGREDIENT_QUANTITY`

Example:

* `listIngredients` (returns chicken as 3rd index)
+
`depleteIngredient i/3 q/10`
...
Edits the staff member with the specified id.

Format: `editStaff STAFF_ID [n/NAME] [p/PHONE] [e/EMAIL] [a/APPOINTMENT]`
...
* *depleteIngredient* `depleteIngredient i/INGREDIENT_INDEX q/INGREDIENT_QUANTITY` +
e.g. `depleteIngredient i/3 q/10`
