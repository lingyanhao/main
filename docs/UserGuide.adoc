...
* All parameters involving time should be specified in `yyyy-MM-dd HH:mm` format, e.g. `2019-02-23 13:00`. If the date is unspecified, the current date will be used. In that situation, `HH:mm` format should be used, e.g. `13:00`.
*
...
• If subscription settings is not specified, it will be assumed that the user is by default subscribed to the system.
...
****
• If the capacity is full, the restaurant will not accept new bookings (refer to <<Create New Booking: `addBooking`, addBooking command >>). If this command causes the number of bookings to exceed the capacity, the `updateCapacity` command will not be executed and an error will be displayed., instead some of the bookings must be cancelled.
****
