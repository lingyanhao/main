...
When a `Member` details (`Name`, `Phone`, or `Email`) is changed, the `Booking` object must reflect the change. This requires an internal modification to the `Booking` list.

==== Current implementation

When `RestaurantBook#setMember()` is called, the bookings are replaced with a new set of bookings, in a way such that any `Booking` containing the modified member will be replaced with another `Booking` object that correctly reflects the modified customer.
...
* **Alternative 3:** Assign each `Member` an ID and store the mapping from ID to `Member`. Instead of storing the full details of the `Member`, the `Booking` object simply stores the `ID` of the member that made the booking. When an update occurs, we simply need to update the ID to `Member` map.
** Pros: Provides both lookup efficiency as well as immutability of the `Member` objects.
** Cons: Uses additional data structure to store the mappings, potentially making the code more difficult to maintain.


* Decision: Alternative 1 was adopted as we felt that it was the simplest way to implement the feature. The additional efficiency gained may not even be significant, considering that in simple GUI apps like this, updating the UI is often the part of the program that uses the most resources. Optimizing the backend operations may not actually lead to a significant performance improvement.
...
In other words, when RestaurantBook#deleteMember() or RestaurantBook#deleteIngredient() is called, this triggers all related bookings or ingredients to be deleted as well.
...
*** Aliases are implemented as a shortcut to the long commands (e.g. `db` for `deleteBooking`, `dm` for `deleteMember`).
...
* Decision: Alternative 1 was adopted as we felt it was the simplest way to implement the feature.
...
When the `stats` command is entered, the current statictis of the amount of bookings is to be displayed to the user in the form of a bar chart.

==== Current implementation

The `RestaurantBookParser` class would detect if the user has keyed in the keyword `stats`. If it does, `RestaurantBookParser` would call the `Statistics` class. The `Statistics` class is used to determine what data is to be displayed to the user in the bar chart (ie what are the axes, values, and legends of the bar chart). It would call the `StatsWindow` class which handles the UI. The bar chart will appear in a new window and does not affect the current window.
...
** Cons: Need to change the implementation of every command that causes a change to the database. Calculations have to be done every operation, affecting efficiency.

* Decision: Alternative 1 was adopted as it is an easier implementation. As the number of times the `stats` command is called is likely to be low as compared to an `add` or `delete` operation, doing the calculation after every operation would be a waste of computation power. 
...
* **Alternative 2:** Hide the displayed database to show the statistics.
...
* **Alternative 1 (current choice):** Making a separate fxml file for each item card.
...
* Decision: Alternative 1 was adopted. Currently, the only difference between the panels are the header which contains the name of the item type. This only affects a single label, which is easy to edit via code. Hence, making them share the fxml reduces the code duplication. If the display of the panels were to be modified in the future, say to add more padding or to change the size, only one fxml file would need to be modified.
...
. Fix `invalidAndValidPersonAddressBook.json`, `typicalPersonsRestaurantBook.json`, `validAddressBook.json` etc., such that the JSON tests will not fail due to a missing `remark` field.
...
1.  User requests to find members and gives a list of words
