...
* **Alternative 2:** Make the methods generic, so only one `addItem` method.
...
When a `Member`'s details (`Name`, `Phone`, or `Email`) is changed, the `Booking` object must reflect the change. This requires an internal modification to the `Booking` list. 
...
** Cons: Poor efficiency. This involves scanning through the entire list of bookings just to change the personal details of 1 `Member`. Also, if some other command changes a `Member`'s details without going through the `RestaurantBook#setMember()` method, the update will not be reflected.

* **Alternative 2:** Make the `Member` class mutable. Whenever the `Member`s details changes, the `Booking` will automatically extract the correct details of the `Member` and nothing needs to be done.
...
* **Alternative 3:** Assign each `Member` an ID and store the mapping from ID to `Member`s. Instead of storing the full details of the `Member`, the `Booking` object simply stores the `ID` of the member that made the booking. When an update occurs, we simply need to update the ID to `Member` map.
