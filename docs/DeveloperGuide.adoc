...
** Cons: Need to change the implementation of every command that causes a change to the database. If the user does not want to display the statistics, the calculations done every operation would be wasted.
...
* **Alternative 1 (current choice):** Using the same fxml file for each item.
** Pros: Less duplication.
** Cons: Hard to customise the contents of each panel. However, currently, the only difference between the panels are the header which contains the name of the item type, and that single label is easy to edit via code.
