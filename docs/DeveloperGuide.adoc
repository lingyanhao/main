...
The following sequence diagram demonstrates how the add feature works.
...
When the `stats` command is entered, the current statistis of the amount of bookings is to be displayed to the user in the form of a bar chart.

==== Current implementation

When the user enters the `stats` command, `RestaurantBookParser` calls the `Statistics` class. The Statistics class fetches the relevant data for user display (ie what are the axes, values, and legends of the bar chart). It would then call the `StatsWindow` class which handles the UI. The bar chart will appear in a new window and does not affect the current window.
